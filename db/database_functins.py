...
def view_all(database_obj):
    postgresql_select_query = "select * from passmanage"
    all_entries = database_obj.fetch(postgresql_select_query)
...
def add_entry(database_obj):
    cursor = database_obj.ps_connection.cursor()
    ...
    try:
        for record in insert_values:
            ...
            database_obj.ps_connection.commit()
...
def delete_entry(database_obj):
    ...
    try:
        cursor.execute("""DELETE FROM public.passmanage WHERE Website = %s AND UserID = %s""",
